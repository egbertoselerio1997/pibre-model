...
- Case: measurable-schema ASM1 implementation validation on Windows. Strategy: run uv run c:/Users/eselerio/projects/pibre-model/.venv/Scripts/python.exe -m unittest tests.bootstrap.test_repo_contract tests.simulation.test_asm1_simulation after refactoring asm1_simulation to measurable compounds and derived fractions. Result: effective for validating the updated schema contract, metadata persistence, and behavior tests after code fixes.
- Case: mechanistic steady-state ASM1 implementation validation on Windows. Strategy: run c:/Users/eselerio/projects/pibre-model/.venv/Scripts/python.exe -m unittest tests.bootstrap.test_repo_contract tests.simulation.test_asm1_simulation after replacing the surrogate dataset generator with the mechanistic steady-state CSTR solver and schema changes. Result: effective for validating repository contract compatibility, simulation metadata updates, and solver-backed behavior checks.
- Case: mechanistic ASM1 public API smoke validation on Windows. Strategy: run c:/Users/eselerio/projects/pibre-model/.venv/Scripts/python.exe -c "from src.models.simulation.asm1_simulation import run_asm1_simulation; ..." for a small in-memory run after tests passed. Result: effective for confirming that the public simulation entry point still returns a dataset and updated schema metadata without writing artifacts.
- Case: explicit-matrix ASM1 validation on Windows. Strategy: run c:/Users/eselerio/projects/pibre-model/.venv/Scripts/python.exe -m unittest tests.bootstrap.test_repo_contract tests.simulation.test_asm1_simulation after exposing Petersen and composition matrices and adding aeration as a seventh process row. Result: effective for validating the matrix bundle contract, metadata updates, and notebook-facing compatibility alias behavior.
