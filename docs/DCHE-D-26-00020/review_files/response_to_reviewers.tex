...
Thank you. The revised manuscript no longer relies on the original CoBRE bilinear formulation, but the underlying phenomenological motivation has been retained in updated form. The new framework uses a partitioned second-order feature map with a blockwise driver:
\[
...
Thank you for this important clarification. This concern applied to the earlier CoBRE interpretation. In the revised manuscript, that claim has been removed because the prior bilinear washout discussion is no longer used; instead, the study is reformulated around a standalone CSTR benchmark with hydraulic retention time as an explicit operating variable (sampled in the range [6.0, 36.0]~h), and with physically admissible steady states enforced through invariant and non-negativity constraints rather than a verbal washout claim. \revhighlight{revDefault}{blue}
...
Thank you. This point is explicitly addressed in the revised manuscript. The deployment subsection now states the coupled relation, defines $\widehat{R} = I_F - \widehat{\Gamma}$, and shows that deployment begins from the raw coupled state:
\[
...
Thank you. The revised manuscript now provides explicit numerical safeguards. The recursive coupled-QP estimator described in Section~2.6 constrains the coupling update by restricting $\Gamma$ to an admissible set $\mathcal{G}$:
\begin{itemize}[nosep]
...
Thank you for identifying this valid concern regarding the commutativity of the Kronecker interaction products. In the revised manuscript, the $\Theta_{cc}$ heatmap presented in Figure~4 and the supplementary atlases (Figures~S1--S4) now display the symmetrized interaction matrix
\[
\Theta_{cc}^{(sym)} = \tfrac{1}{2}\bigl(\Theta_{cc} + \Theta_{cc}^T\bigr)
\]
rather than the raw ordered coefficients. The largest-magnitude symmetrized coefficients for COD are:
\begin{itemize}[nosep]
...
\end{itemize}
The interpretability discussion in Section~4.4 refers to ``symmetric negative cross-interactions,'' confirming that pairwise effects are reported from the symmetrized tensor. This ensures that the physical interpretation of commutative second-order terms is mathematically valid. \revhighlight{revRtwoOne}{purple}
...
Thank you. This concern motivated a central redesign of the manuscript. The revised manuscript now develops ICSOR specifically to enforce exact stoichiometric mass conservation together with component non-negativity. The conservation law is derived from the stoichiometric matrix:
\[
...
Thank you. This point has been addressed directly. The revised manuscript introduces the invariant matrix construction from $A\nu^T = 0$, states the conservation relation $Ac_{out} = Ac_{in}$, and enforces the corresponding equality constraints during deployment together with non-negativity constraints. Specifically, the deployment LP (Eq.~11 in the revised manuscript) solves:
\[
