...
That same affine-core relation dictates the epistemology of the framework. Stacking the single-sample expression $y_{aff} = M \phi(u, c_{in}) + H c_{in}$ over a dataset of $N$ steady-state samples yields:
\begin{equation}
Y = \Phi M^T + C_{in} H^T + E
\end{equation}
where $\Phi \in \mathbb{R}^{N \times D}$ is the feature matrix, $Y \in \mathbb{R}^{N \times K}$ the measured output matrix, $C_{in} \in \mathbb{R}^{N \times F}$ the influent state matrix, and $E \in \mathbb{R}^{N \times K}$ the residual matrix. Measured composite data therefore uniquely identify the affine core $M$ via the least-squares pseudoinverse estimator
\begin{equation}
\widehat{M}^T = \Phi^+\left(Y - C_{in} H^T\right)
\end{equation}
However, they do \textit{not} uniquely identify the latent component-space matrix $B$. If $N_B \in \mathbb{R}^{F \times D}$ satisfies $G N_B = 0$, then $G(B + N_B) = M$. 
...
\begin{table}[htbp]
\centering
\caption{Effective test metrics at the full training size (8{,}000 training samples, 2{,}000 test samples), sorted by effective test RMSE rank. ICSOR uses the projected prediction $y^* = I_{comp}\,c^*$; all classical models use their raw measured-space output.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{revision_plots/average_rank_heatmap_20260407_165106.png}
...
\includegraphics[width=\textwidth]{revision_plots/effective_rmse_learning_curve_20260407_165106.png}
...
\includegraphics[width=\textwidth]{revision_plots/effective_mae_learning_curve_20260407_165106.png}
...
\includegraphics[width=\textwidth]{revision_plots/effective_r2_learning_curve_20260407_165106.png}
...
\includegraphics[width=\textwidth]{revision_plots/effective_mape_learning_curve_20260407_165106.png}
...
\includegraphics[width=\textwidth]{revision_plots/effective_rmse_by_target_heatmap_20260407_165106.png}
...
\includegraphics[width=\textwidth]{revision_plots/negative_prediction_rate_by_target_heatmap_20260407_165106.png}
...
\includegraphics[width=0.85\textwidth]{revision_plots/effective_rmse_generalization_gap_20260407_165106.png}
...
However, this near-absence of negatives carries a fundamentally different epistemological status for ICSOR than for the classical regressors. For ICSOR, zero negatives is guaranteed by construction: the quadratic program in Eq.~(6) enforces $c^* \ge 0$ in the latent component space, and the entrywise non-negativity of the relevant rows of $I_{comp}$ then ensures $y^* = I_{comp}\,c^* \ge 0$ in the measured output space. For the remaining models, the near-zero negative rate is still a data-dependent observation that could change under different input distributions, smaller training sets, or extrapolation beyond the simulation domain.
...
\includegraphics[width=\textwidth]{revision_plots/raw_to_effective_adjustment_learning_curve_20260407_165106.png}
...
\includegraphics[width=\textwidth]{revision_plots/effective_constraint_residual_learning_curve_20260407_165106.png}
...
\includegraphics[width=\textwidth]{revision_plots/w_u_heatmap_20260407_060250.png}
\caption{$W_{u,y}$}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
...
\includegraphics[width=\textwidth]{revision_plots/w_in_heatmap_20260407_060250.png}
\caption{$W_{in,y} + H$}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
...
\includegraphics[width=\textwidth]{revision_plots/bias_bar_chart_20260407_060250.png}
\caption{$b_y$}
\end{subfigure}
...
\includegraphics[width=\textwidth]{revision_plots/theta_uu_heatmaps_20260407_060250.png}
\caption{$\Theta_{uu,y}$}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
...
\includegraphics[width=\textwidth]{revision_plots/theta_uc_heatmaps_20260407_060250.png}
\caption{$\Theta_{uc,y}$}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
...
\includegraphics[width=\textwidth]{revision_plots/theta_cc_heatmaps_20260407_060250.png}
\caption{$\Theta_{cc,y}$}
\end{subfigure}
...
In contrast, the one-shot full-size benchmark yields zero negative predictions for all classical models, but the repeated sweep shows that this behavior is not exact across random splits: at 8{,}000 training samples PLS retains a mean negative rate of 0.0008\%, while the remaining classical regressors stay at 0.0\%. The observation is therefore still data-dependent rather than structurally guaranteed. Under more extreme input conditions or smaller training sets, any unconstrained regressor could in principle yield negative effluent concentrations, a failure mode that ICSOR prevents by construction through the non-negativity constraint $c^* \ge 0$ in the QP formulation (Eq.~6) and the entrywise non-negative structure of $I_{comp}$.
